---/// UI
local Framework = loadstring(game:HttpGet("https://pastebin.com/raw/mRvnKmR2"))()
...
if not isfolder('Future Client') then
    makefolder('Future Client')
	makefolder('Future Client\\Data')
	makefolder('Future Client\\Cache')
	makefolder('Future Client\\Audio')
	makefolder('Future Client\\Configuration')
	-- Begin
	writefile('Future Client\\Data\\readme.txt', [[
		Welcome!
		Do not touch anything as it can break your Future Client Experience.
	]])
	writefile('Future Client\\Data\\stitch.txt', [[
		(<3)
		;stitch; x3
	]])
    writefile('Future Client\\Data\\config.cfg', [[
		(<3)
		;stitch; x3
	]])
	writefile('Future Client\\Cache\\readme.txt', [[
		Welcome!
		Do not touch anything as it can break your Future Client Experience.
	]])
	writefile('Future Client\\Audio\\readme.txt', [[
		Welcome!
		To create a playlist, make a new folder in this directory then drag in any .mp3 files.
		Keep names short!
	]])
	writefile('Future Client\\Configuration\\confighelp.txt', [[
		Welcome!
		Drag in any JSON (.json) files in here if you would like custom Future Client configs.
	]])
	writefile('Future Client\\welcome.txt', [[
		== Thank you for using Future Client! ==
		For any support you may need, feel free to join our Discord community: discord.gg/9nY7VEHw3s.
	]])
end
local Library = {
	LibraryMetadata = {
	...
	JSON = {
		WriteData = function(name, data)
			local file = 'Future Client\\Data\\'..name..".json"
		...
		DataExist = function(name)
			return isfile('Future Client\\Data\\'..name..'.json')
		end
	},
Framework = {
    -- Allow dragging functionality
    ...
    -- Allow resizing functionality
    AllowResizing = function(Object, Frame, Speed)
        local resizeToggle = false
        ...
        -- Update function to resize the frame
        local function updateResize(input)
            if not Util.Enabled then return end
            local delta = input.Position - initialPos
            local newSize = UDim2.new(
                0, initialSize.X.Offset + delta.X, -- resize horizontally
                0, initialSize.Y.Offset + delta.Y  -- resize vertically
            )
            -- Smooth resizing with tweens
            game:GetService("TweenService"):Create(Frame, TweenInfo.new(Speed), {
                Size = newSize
            }):Play()
        end
        ...
        Object.InputBegan:Connect(function(input)
            if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) then
                local isNearRightEdge = input.Position.X >= Frame.AbsolutePosition.X + Frame.AbsoluteSize.X - 10
                local isNearBottomEdge = input.Position.Y >= Frame.AbsolutePosition.Y + Frame.AbsoluteSize.Y - 10
                
                if isNearRightEdge or isNearBottomEdge then
                    resizeToggle = true
        ...
        game:GetService("UserInputService").InputChanged:Connect(function(input)
            if input == dragInput and resizeToggle then
                updateResize(input)
            end
        end)
...
local sessionStartTime = os.time() -- Record the session start time

-- Function to get the session duration
local function getSessionDuration()
    local currentTime = os.time()
    local duration = currentTime - sessionStartTime -- Calculate duration in seconds
...
-- Create the session duration label
local sessionDurationLabel = createTextLabel(GProfile, "Session Duration: " .. getSessionDuration(), UDim2.new(0.296, 0, 0.355, 0))

-- Function to update the session duration label every second
local function updateSessionDuration()
    while true do
        sessionDurationLabel.Text = "Playtime: " .. getSessionDuration()
        wait(1) -- Update every second
    end
end

-- Start the duration update in a coroutine
coroutine.wrap(updateSessionDuration)()

-- Check if a previous session time exists
if _G.sessionStartTime then
    sessionStartTime = _G.sessionStartTime -- Use the stored time if available
end

-- Store the session start time globally
_G.sessionStartTime = sessionStartTime

-- Optional: Connect to teleport event to reset timer if needed
game.Players.LocalPlayer.CharacterAdded:Connect(function()
    sessionStartTime = os.time() -- Reset on respawn
    _G.sessionStartTime = sessionStartTime -- Update the global session start time
end)
	...
	local UpperSection = CreateObject("Frame", {
		Name = "UpperSection",	Parent = TabFrame,	BackgroundColor3 = Color3.fromRGB(255,255,255),	BackgroundTransparency = 1,	Position = UDim2.new(0.0025906735099852085,0,0,0),	Size = UDim2.new(0,450,0,45),	Transparency = 1,
	})
	...
	local Drag = CreateObject("ImageButton", {
		Name = "Drag",	Parent = UpperSection,	AnchorPoint = Vector2.new(0,0.5),	BackgroundColor3 = Color3.fromRGB(255,255,255),	BackgroundTransparency = 1,	Position = UDim2.new(0.999999999545558,0,0.5,0),	Size = UDim2.new(0,10,0,10),	AutoButtonColor = false,	Image = "http://www.roblox.com/asset/?id=5172066892",	ImageTransparency = 0.5,
	})
	local TabTitle = CreateObject("TextLabel", {
		Name = "TabTitle",	Parent = UpperSection,	BackgroundColor3 = Color3.fromRGB(255,255,255),	BackgroundTransparency = 1,	Position = UDim2.new(0.04675324633717537,0,0,0),	Size = UDim2.new(0,182,0,46),	Font = Enum.Font.SourceSans,	Text = "Home",	TextColor3 = Color3.fromRGB(255,255,255),	TextSize = 18,	TextTransparency = 0.25999999046325684,	TextXAlignment = Enum.TextXAlignment.Left,
	})
	...
	local ConfigurationalValues = CreateObject("Folder", {
		Name = "ConfigurationalValues",	Parent = ClientCore,
	})
	...
	local TextQ = CreateObject("TextLabel", {
		Name = "Title", Parent = Question,	AnchorPoint = Vector2.new(0.5,0.5),	BackgroundColor3 = Color3.fromRGB(255,255,255),	BackgroundTransparency = 1,	Position = UDim2.new(0.5,0,0.46050596237182617,0),	Size = UDim2.new(0,200,0,36),	Font = Enum.Font.SourceSans,Text = "Would to exit Future Client?",	TextColor3 = Color3.fromRGB(255,255,255),	TextSize = 18,
	})
    ...
    if not JSON.DataExist('Config') then
		JSON.WriteData('config', {
			"possible configs"
		})
	end
...
local function minimizeUI()
    if Minimizing or Maximizing then return end
    Minimized = true
    Minimizing = true
    ...
    if _G.minimizeAllGuis then
        -- Hide all relevant GUI elements
        FrameMain.Visible = false
        Sidebar.Visible = false
        -- Add any additional frames that should be hidden here
    else
        -- Tweening the size of the main frame (if you want it to shrink visually)
        TweenObject(FrameMain, Enum.EasingStyle.Circular, 0.3, Enum.EasingDirection.Out, {
            Size = UDim2.new(0, 53, 0, 28)
        })
        wait(0.3)

        -- Hide sidebar frames if necessary
...
local function maximizeUI()
    ...
    -- Check the global toggle state
    if _G.minimizeAllGuis then
        -- Show all relevant GUI elements
        FrameMain.Visible = true
        Sidebar.Visible = true
        -- Add any additional frames that should be shown here
    else
        -- Tweening the size of the main frame back to its original size
        TweenObject(FrameMain, Enum.EasingStyle.Circular, 0.3, Enum.EasingDirection.Out, {
            Size = UDim2.new(0, 565, 0, 419)
        })
        wait(0.3)

        -- Show sidebar frames if necessary
        for _, v in pairs(Sidebar:GetChildren()) do
            if v:IsA('Frame') and v.Name ~= 'AControlModule' then
                v.Visible = true
            end
...
Minimize.MouseButton1Down:Connect(minimizeUI)

-- Maximize on button click
Maximize.MouseButton1Down:Connect(maximizeUI)
...
Exit.MouseButton1Down:Connect(function()
    ...
    WinHandle.AskQuestion('Are you sure you would like to exit Future Client?', function(a)
        if a == true then
            WinHandle.Exit()
        end
    end)
...
-- Keybind to toggle minimize/maximize UI
UserInputService.InputBegan:Connect(function(input, gameProcessedEvent)
    if gameProcessedEvent then return end  -- Ignore if the game is processing the input (like typing in a text box)

    -- Check for the RightShift key
    if input.UserInputType == Enum.UserInputType.Keyboard and input.KeyCode == Enum.KeyCode.RightShift then
        if Minimized then
            maximizeUI()  -- Maximize if currently minimized
        else
            minimizeUI()  -- Minimize if currently maximized
        end
    end
end)
	...
	function WinHandle.Tab(name, img)
		...
		local TabButton = CreateObject("TextButton", {
			Name = "TabButton",	Parent = ETabs,	BackgroundColor3 = Color3.fromRGB(255,255,255),	BackgroundTransparency = 1,	Position = UDim2.new(0.09612662345170975,0,0,0),	Size = UDim2.new(0.8077465891838074,0,0.014869890175759792,25),	AutoButtonColor = false,	Font = Enum.Font.SourceSans, Text = (name or "Tab"),	TextColor3 = Color3.fromRGB(255,255,255),	TextSize = 18,	TextTransparency = 0.7,	TextXAlignment = Enum.TextXAlignment.Left,
		})
		local UICorner = CreateObject("UICorner", {
		...
		function SectionHandle.Section(side)
			local Section = CreateObject("Frame", {
				Name = "Section",	Parent = (side == 'left' and Left or Right),	AutomaticSize = Enum.AutomaticSize.Y,	BackgroundColor3 = Color3.fromRGB(255,255,255),	BackgroundTransparency = 0.949999988079071,	Size = UDim2.new(1,0,-0.51585041880607605,58),	Transparency = 0.949999988079071,
			})
			local UICorner = CreateObject("UICorner", {
    ...
    local TextButton = CreateObject("TextButton", {
        Parent = Button, AnchorPoint = Vector2.new(0.5, 0.5), BackgroundColor3 = Color3.fromRGB(255, 255, 255), BackgroundTransparency = 0.9700000286102295, Position = UDim2.new(0.4969879388809204, 0, 0.5, 0), Size = UDim2.new(0.949999988079071, 0, 0, 23),
        AutoButtonColor = false, Font = Enum.Font.SourceSans, Text = name, TextColor3 = Color3.fromRGB(255, 255, 255), TextSize = 18, TextTransparency = 0.5, TextXAlignment = Enum.TextXAlignment.Left,
    })
    ...
    TextButton.MouseButton1Down:Connect(function()
        -- Tween effect to highlight button
        TweenObject(TextButton, Enum.EasingStyle.Quad, 0.3, Enum.EasingDirection.Out, {
            BackgroundTransparency = 0.85  -- Lighter effect
        })

        -- Execute callback function
        task.spawn(callback)

        -- Restore background transparency after click with a smooth transition
        wait(0.18)  -- Time to keep the highlight effect before returning to normal
        TweenObject(TextButton, Enum.EasingStyle.Quad, 0.3, Enum.EasingDirection.In, {
            BackgroundTransparency = 0.97  -- Original transparency
        })
    end)
...
function AssetHandler.Slider(name, min, max, default, symbol, callback)
    ...
    local TextLabel = CreateObject("TextLabel", {
        ...
        BackgroundColor3 = Color3.fromRGB(255, 255, 255),
        BackgroundTransparency = 1,
        Position = UDim2.new(0.007, 0, 0.067, 0),
    ...
    local BG = CreateObject("Frame", {
        ...
        BorderSizePixel = 0,
        Position = UDim2.new(0.285, 0, 0.5, 0),
        Size = UDim2.new(0, 77, 0, 2),
        Transparency = 0.75,
    })
    ...
    local Frame = CreateObject("Frame", {
        ...
        Position = UDim2.new(0.280, 0, 0.300, 0),
        Size = UDim2.new(0, 77, 0, 12),
        Transparency = 1,
    })
    ...
    local Amount = CreateObject("TextBox", { -- it's a TextBox now
        Name = "Amount",
        Parent = TextButton,
        AnchorPoint = Vector2.new(0, 0.5),
        BackgroundColor3 = Color3.fromRGB(255, 255, 255),
        BackgroundTransparency = 0.95,
        Position = UDim2.new(0.801, 0, 0.5, 0),
    ...
    -- ✅ Correct: Auto-select text on Focused event
    Amount.Focused:Connect(function()
        Amount.CursorPosition = #Amount.Text + 1
        Amount.SelectionStart = 1
    end)

    -- Handle user input after editing
    Amount.FocusLost:Connect(function(enterPressed)
        if enterPressed then
            local input = tonumber(Amount.Text:match("%d+"))
            if input then
                input = math.clamp(input, min, max)
                slider:set(input)
                Amount.Text = input .. symbol
                task.spawn(function()
                    pcall(function()
                        callback(input)
                    end)
                ...
                Amount.Text = tostring(slider:get()) .. symbol
            end
        end
    end)
    ...
    return Handle
			...
			function AssetHandler.Keybind(name, Dkey, callback, onchanged)
				...
				local TextButton = CreateObject("TextButton", {
					Parent = Keybind,	BackgroundColor3 = Color3.fromRGB(255,255,255),	BackgroundTransparency = 1,	Size = UDim2.new(1,0,1,0),	Font = Enum.Font.SourceSans,	Text = name,	TextColor3 = Color3.fromRGB(255,255,255),	TextSize = 16,	TextTransparency = 0.5,	TextXAlignment = Enum.TextXAlignment.Left,
				})
				local UIPadding = CreateObject("UIPadding", {
					Parent = TextButton,	PaddingLeft = UDim.new(0,5),
				})
				local Amount = CreateObject("TextLabel", {
					Name = "Amount",	Parent = TextButton,	AnchorPoint = Vector2.new(0.8899999856948853,0.5),	AutomaticSize = Enum.AutomaticSize.X,	BackgroundColor3 = Color3.fromRGB(255,255,255),	BackgroundTransparency = 0.949999988079071,	Position = UDim2.new(0.9427019953727722,0,0.5,0),	Size = UDim2.new(0,3,0,15),	Font = Enum.Font.SourceSans,	Text = "End",	TextColor3 = Color3.fromRGB(202,202,202),	TextSize = 11,	TextWrapped = true,	TextXAlignment = Enum.TextXAlignment.Right,
				})
				local UICorner = CreateObject("UICorner", {
...
function AssetHandler.ToggleWithKeybind(name, defaultState, callback, Dkey)
    ...
    -- Adjust splash position to be further right
    local Splash = CreateObject("Frame", {
        Parent = TextButton,
        AnchorPoint = Vector2.new(0, 0.5),
        BackgroundColor3 = Color3.fromRGB(255,255,255),
        BackgroundTransparency = 0.9,
        Position = UDim2.new(0.78, 0, 0.5, 0), -- Slightly adjusted to the right
        Size = UDim2.new(0, 30, 0, 15),
    })
    CreateObject("UICorner", {Parent = Splash, CornerRadius = UDim.new(1,0)})
    ...
    local Amount = CreateObject("TextLabel", {
        ...
        Position = UDim2.new(0.6, 0, 0.5, 0), -- Moved slightly left
    ...
    CreateObject("UICorner", {Parent = Amount, CornerRadius = UDim.new(0,4)})
    ...
    -- Local function to update toggle state
    local function UpdateToggleVisual()
        if State then
            TweenObject(Dot, Enum.EasingStyle.Linear, 0.1, Enum.EasingDirection.Out, {
    ...
    -- Local function to toggle the state
    ...
    -- Mouse button click handling (to toggle state)
    TextButton.MouseButton1Down:Connect(function(x, y)
        -- Only toggle the state if the left part of the button or splash is clicked
        if x < TextButton.AbsoluteSize.X * 0.75 or x > Splash.AbsolutePosition.X and x < Splash.AbsolutePosition.X + Splash.AbsoluteSize.X then
            SetToggleState(not State)  -- Toggle the state if clicked on the left side or splash
        else
            -- Do nothing (keybind display area)
            Amount.Text = "..."
        end
    end)

    -- Keybind setup (ensure it's separate from other keybinds)
    local key = Framework.Keybind(TextButton, { Enum.KeyCode.Space, Enum.KeyCode.W, Enum.KeyCode.A, Enum.KeyCode.S, Enum.KeyCode.D, Enum.KeyCode.LeftShift, Enum.KeyCode.LeftControl, Enum.UserInputType.MouseMovement, Enum.UserInputType.MouseButton1, Enum.UserInputType.MouseButton2 })

    -- Ensure that keybind actions are isolated (no interference with other codes)
    key.Updated:Connect(function(newKey)
        Amount.Text = tostring(newKey.Name)
    end)

    -- Trigger toggle when keybind is pressed
    key.Pressed:Connect(function()
        SetToggleState(not State)  -- Toggle the state when the keybind is pressed
        Amount.BackgroundTransparency = 0.9  -- Highlight keybind button
        task.wait()
        Amount.BackgroundTransparency = 0.949999988079071  -- Reset transparency
    end)

    key:set(Dkey or Enum.KeyCode.Q)  -- Default keybind
    ...
    task.defer(UpdateToggleVisual)
			...
			function AssetHandler.ColorPicker(name, preset, callback)
				...
				local TextButton = CreateObject("TextButton", {
					Parent = ColorPicker,	AnchorPoint = Vector2.new(0.5,0.5),	BackgroundColor3 = Color3.fromRGB(255,255,255),	BackgroundTransparency = 0.9700000286102295,	Position = UDim2.new(0.5,0,0.5,0),	Size = UDim2.new(0.95,0,0.699999988079071,0),	AutoButtonColor = false,	Font = Enum.Font.SourceSans,	Text = "Color",	TextColor3 = Color3.fromRGB(255,255,255),	TextSize = 15,	TextTransparency = 0.5,	TextXAlignment = Enum.TextXAlignment.Left,
				})
			...
			function AssetHandler.Label(text)
				...
				local TextButton = CreateObject("TextButton", {
					Parent = Label,	BackgroundColor3 = Color3.fromRGB(255,255,255),	BackgroundTransparency = 1,	Size = UDim2.new(1,0,1,0),	Font = Enum.Font.SourceSans,	Text = text,	TextColor3 = Color3.fromRGB(255,255,255),	TextSize = 15,	TextTransparency = 0.5,	TextXAlignment = Enum.TextXAlignment.Left,
				})
				local UIPadding = CreateObject("UIPadding", {
					Parent = TextButton,	PaddingLeft = UDim.new(0,5),
				})
...
function AssetHandler.Dropdown(name, default, list, callback)
    local Dropdown = CreateObject("Frame", {
        ...
        BackgroundColor3 = Color3.fromRGB(255, 255, 255),
        BackgroundTransparency = 1,
        Position = UDim2.new(0, 0, 0.7777777910232544, 0),
        Size = UDim2.new(1, 0, 0, 38),
        Transparency = 1,
    })
    
    local SelectedOption = default or list[1] -- Set default or first item as the selected option
    local DisplayText = name .. ": " .. SelectedOption -- Use the dropdown name here
    ...
    local OpenBtn = CreateObject("TextButton", {
        ...
        Size = UDim2.new(0.949999988079071, 0, 0.699999988079071, 0),
    ...
    local UIStroke = CreateObject("UIStroke", {
        ...
        Transparency = 0.925000011920929,
    })
    ...
    local Dropper = CreateObject("Frame", {
        ...
        BackgroundColor3 = Color3.fromRGB(255, 255, 255),
        BackgroundTransparency = 1,
        Position = UDim2.new(0, 0, 0.5938987731933594, 0),
    ...
    local TextButton = CreateObject("TextButton", {
        ...
        BackgroundColor3 = Color3.fromRGB(255, 255, 255),
        BackgroundTransparency = 0.9700000286102295,
        Position = UDim2.new(0.5, 0, 0.4338647127151489, 2),
        Size = UDim2.new(0.949999988079071, 0, 0.8679999709129333, 0),
        AutoButtonColor = false,
        Font = Enum.Font.SourceSans,
        Text = "",
        TextColor3 = Color3.fromRGB(255, 255, 255),
    ...
    local ScrollingFrame = CreateObject("ScrollingFrame", {
        ...
        BackgroundColor3 = Color3.fromRGB(255, 255, 255),
        BackgroundTransparency = 1,
        Position = UDim2.new(0.5, 0, 0.519342303276062, 0),
        Size = UDim2.new(1, 0, 0.9424876570701599, 0),
        Transparency = 1,
    ...
    callback(SelectedOption) -- Initial callback with the default option
    ...
    for i, v in pairs(list) do
        local DropButton = CreateObject("TextButton", {
            ...
            AnchorPoint = Vector2.new(0.5, 0.5),
            BackgroundColor3 = Color3.fromRGB(255, 255, 255),
            BackgroundTransparency = (v == SelectedOption and 0.9 or 0.9700000286102295),
            Position = UDim2.new(0.4870184361934662, 0, 0.18865394592285156, 0),
            Size = UDim2.new(0.974037230014801, -3, 0, 23),
            AutoButtonColor = false,
            Font = Enum.Font.SourceSans,
            Text = v,
            TextColor3 = Color3.fromRGB(255, 255, 255),
        ...
        DropButton.MouseButton1Down:Connect(function()
            for _, button in pairs(ScrollingFrame:GetChildren()) do
                if button:IsA('TextButton') then
                    TweenObject(button, Enum.EasingStyle.Linear, 0.1, Enum.EasingDirection.Out, {
                        BackgroundTransparency = 0.9700000286102295
                    })
                end
            end
            ...
            -- Update selected option and display text
            SelectedOption = v
            DisplayText = name .. ": " .. SelectedOption -- Update to use the dropdown name
            OpenBtn.Text = DisplayText -- Update button text
            callback(v) -- Call the callback with the new selected value
        end)
    end
    ...
    return Set
			...
			function AssetHandler.Textbox(name, preset, callback)
				...
				local TextButton = CreateObject("TextButton", {
					Parent = Textbox,	BackgroundColor3 = Color3.fromRGB(255,255,255),	BackgroundTransparency = 1,	Size = UDim2.new(1,0,0,20),	Font = Enum.Font.SourceSans,	Text = name,	TextColor3 = Color3.fromRGB(255,255,255),	TextSize = 15,	TextTransparency = 0.5,	TextXAlignment = Enum.TextXAlignment.Left,
				})
				local UIPadding2 = CreateObject("UIPadding", {
			...
			function AssetHandler.Toggle(name, state, callback)
				...
				Toggle.Name = name
				local TextButton = CreateObject("TextButton", {
					Parent = Toggle,	BackgroundColor3 = Color3.fromRGB(255,255,255),	BackgroundTransparency = 1,	Size = UDim2.new(1,0,1,0),	Font = Enum.Font.SourceSans,	Text = name,	TextColor3 = Color3.fromRGB(255,255,255),	TextSize = 17,	TextTransparency = 0.5,	TextXAlignment = Enum.TextXAlignment.Left,
				})
				local UIPadding = CreateObject("UIPadding", {
					Parent = TextButton,	PaddingLeft = UDim.new(0,5),
				})
...
function Library.Watermark()
	...
	local TextLabel = CreateObject("TextLabel", {
		Parent = Frame,	BackgroundColor3 = Color3.fromRGB(255,255,255),	BackgroundTransparency = 1,	Position = UDim2.new(0,25,0,0),	Size = UDim2.new(-2.78125,200,1,0),	Font = Enum.Font.JosefinSans,	Text = "Future",	TextColor3 = Color3.fromRGB(255,255,255),	TextSize = 11,	TextTransparency = 0.4399999976158142,	TextXAlignment = Enum.TextXAlignment.Left,
	})
	local UIPadding = CreateObject("UIPadding", {
...
if not _G.Notification then
    _G.activeNotifications = {}
    _G.notificationHeight = 85
    _G.notificationSpacing = 10
    _G.startY = 50

    function _G:Notification(title, desc, folder, visibletime)
        task.spawn(function()
            pcall(function()
                -- Create screenGui
                ...
                instances.Frame_1.BackgroundColor3 = Color3.fromRGB(14, 14, 14)
                instances.Frame_1.BorderSizePixel = 0
                instances.Frame_1.Position = UDim2.new(0, -250, 0, _G.startY + (#_G.activeNotifications * (_G.notificationHeight + _G.notificationSpacing)))
                instances.Frame_1.Size = UDim2.new(0, 250, 0, _G.notificationHeight)

                instances.UICorner_2.Parent = instances.Frame_1
                instances.UICorner_2.CornerRadius = UDim.new(0, 8)

                -- Text Labels
                instances.TextLabel_1.Parent = instances.Frame_1
                instances.TextLabel_1.Font = Enum.Font.GothamBold
                ...
                instances.TextLabel_1.Position = UDim2.new(0, 10, 0, 5)
                instances.TextLabel_1.Size = UDim2.new(1, -20, 0, 20)
                ...
                instances.TextLabel_2.TextColor3 = Color3.fromRGB(150, 150, 150)
                instances.TextLabel_2.TextSize = 17
                instances.TextLabel_2.TextXAlignment = Enum.TextXAlignment.Left
                instances.TextLabel_2.BackgroundTransparency = 1
                instances.TextLabel_2.Position = UDim2.new(0, 10, 0, 25)
                instances.TextLabel_2.Size = UDim2.new(1, -20, 0, 20)

                instances.TextLabel_3.Parent = instances.Frame_1
                instances.TextLabel_3.Font = Enum.Font.SourceSans
                instances.TextLabel_3.Text = folder
                instances.TextLabel_3.TextColor3 = Color3.fromRGB(150, 150, 150)
                instances.TextLabel_3.TextSize = 17
                instances.TextLabel_3.TextXAlignment = Enum.TextXAlignment.Left
                instances.TextLabel_3.BackgroundTransparency = 1
                instances.TextLabel_3.Position = UDim2.new(0, 10, 0, 45)
                instances.TextLabel_3.Size = UDim2.new(1, -20, 0, 20)

                -- Progress Bar
                instances.ProgressBar.Parent = instances.Frame_1
                instances.ProgressBar.BackgroundColor3 = Color3.fromRGB(60, 60, 60)
                instances.ProgressBar.Size = UDim2.new(1, -20, 0, 5)
                instances.ProgressBar.Position = UDim2.new(0, 10, 0, 70)

                instances.ProgressFill.Parent = instances.ProgressBar
                instances.ProgressFill.BackgroundColor3 = Color3.fromRGB(0, 255, 0)
                instances.ProgressFill.Size = UDim2.new(0, 0, 1, 0)

                local targetY = _G.startY + (#_G.activeNotifications * (_G.notificationHeight + _G.notificationSpacing))
                screenGui.Parent = game:GetService("CoreGui")
                instances.Frame_1:TweenPosition(UDim2.new(0, 10, 0, targetY), "InOut", "Sine", 0.5, true)

                -- Create progress bar animation
                local progressTween = game:GetService("TweenService"):Create(instances.ProgressFill, TweenInfo.new(visibletime, Enum.EasingStyle.Linear, Enum.EasingDirection.InOut), {Size = UDim2.new(1, 0, 1, 0)})
                progressTween:Play()

                -- Add notification to active notifications list
                table.insert(_G.activeNotifications, {gui = screenGui, frame = instances.Frame_1})

                -- Run cleanup asynchronously
                task.spawn(function()
                    wait(visibletime)
                    -- Cleanup after notification time
                    instances.Frame_1:TweenPosition(UDim2.new(-1, 0, 0, targetY), "InOut", "Sine", 0.6, true)
                    instances.ProgressFill:TweenSize(UDim2.new(0, 0, 1, 0), "InOut", "Sine", 0.3, true)
                    wait(0.3)

                    -- Remove from active notifications and destroy the notification
                    for i, notifData in pairs(_G.activeNotifications) do
                        if notifData.gui == screenGui then
                    ...
                    -- Adjust position of other notifications
                    for i, notifData in pairs(_G.activeNotifications) do
                        local newY = _G.startY + ((i - 1) * (_G.notificationHeight + _G.notificationSpacing))
                        notifData.frame:TweenPosition(UDim2.new(0, 10, 0, newY), "InOut", "Sine", 0.5, true)
                    end
                end)
            end)
        end)
